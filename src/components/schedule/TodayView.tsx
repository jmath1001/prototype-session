...
import { PlusCircle, Check, Clock, Calendar as CalendarIcon, X, Loader2, RotateCcw } from 'lucide-react';
...
function SidePanel({
  todayIso, sessions, tutors, daySessions, dayLabel, pendingStudents,
  setSelectedSessionWithNotes, refetch,
}: {
  todayIso: string; sessions: any[]; tutors: any[]; daySessions: any[];
  dayLabel: string; pendingStudents: any[];
  setSelectedSessionWithNotes: (s: any) => void; refetch: () => void;
}) {
  ...
  const filteredAtt = attFilter === 'all' ? allToday.filter(s => s.status !== 'cancelled')
        ...
        <div className="flex shrink-0" style={{ background: '#f3f4f6', borderBottom: '1px solid #e5e7eb' }}>
          {([
            ...
            <button key={t.key} onClick={() => setTab(t.key)}
              ...
              style={tab === t.key ? { background: 'white', color: '#111827', borderBottom: '2px solid #1f2937' } : { color: '#9ca3af', borderBottom: '2px solid transparent' }}>
              {t.label}
              {t.count > 0 && (
                ...
                {pendingStudents.map((student: any, idx: number) => (
                  <div key={`${student.rowId || student.id}-${idx}`}
                    ...
                    onClick={() => setSelectedSessionWithNotes({
                      ...student.session, activeStudent: student, dayName: dayLabel, date: todayIso,
                      tutorName: student.tutorName, block: daySessions.find((b: any) => b.time === student.sessionTime),
                    })}>
            ...
            <div className="flex gap-1 px-2 py-2 shrink-0" style={{ borderBottom: '1px solid #f3f4f6' }}>
              {([
                ...
                <button key={f.key} onClick={() => setAttFilter(f.key)}
                  ...
                  style={attFilter === f.key ? { background: '#1f2937', color: 'white' } : { background: '#f3f4f6', color: '#6b7280' }}>
                  <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ background: attFilter === f.key ? 'white' : f.dot }} />
                  {f.label}
                  {attCounts[f.key] > 0 && <span style={{ opacity: 0.7 }}>({attCounts[f.key]})</span>}
                </button>
              ))}
            </div>
            <div className="overflow-y-auto flex-1 p-2">
              {filteredAtt.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full gap-2 py-8">
                  <p className="text-[10px] font-semibold text-center" style={{ color: '#9ca3af' }}>No {attFilter === 'all' ? '' : attFilter} students</p>
                </div>
              ) : (
                <div className="space-y-1.5">
                  {filteredAtt.map((student: any, idx: number) => {
                    ...
                    return (
                      <div key={`${student.rowId || student.id}-${idx}`} className="rounded-lg overflow-hidden"
                        ...
                        <div className="flex border-t" style={{ borderColor: st.border }}>
                          {([
                            ...
                            return (
                              <button key={btn.status} disabled={isToggling}
...
export function TodayView({
  tutors, sessions, timeOff, students, selectedTutorFilter, tutorPaletteMap,
  setSelectedSessionWithNotes, handleGridSlotClick, onInlineBook, refetch, selectedDate, onDateChange,
}: TodayViewProps) {
  ...
  const pendingStudents = sessions
    ...
    .flatMap(s => s.students
      ...
      .map((st: any) => ({ ...st, sessionTime: s.time, tutorName: tutors.find(t => t.id === s.tutorId)?.name ?? '', session: s }))
    )
  ...
  const handleSave = async (key: string, tutor: Tutor, block: any) => {
    ...
    try {
      await onInlineBook({
        tutorId: tutor.id, date: todayIso, time: block.time,
        student: form.student, topic: form.topic,
        recurring: form.recurring, recurringWeeks: form.recurringWeeks,
      });
      closeForm(key);
      logEvent('session_booked', { studentName: form.student.name, date: todayIso, recurring: form.recurring, recurringWeeks: form.recurringWeeks, source: 'inline_today' });
      refetch();
    } catch (err: any) {
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.value) { onDateChange(new Date(e.target.value + 'T00:00:00')); setForms({}); setOpenDropdown(null); }
  };
  ...
  const renderInlineForm = (tutor: Tutor, block: any, palette: any) => {
    ...
    return (
      <div data-inline-form className="flex flex-col gap-2 p-2.5 rounded-xl"
        style={{ background: 'white', border: '1.5px solid #6366f1', boxShadow: '0 2px 14px rgba(99,102,241,0.13)', minHeight: 100 }}>
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => patchForm(key, { recurring: !form.recurring })}
            className="flex items-center gap-1.5 px-2 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider transition-all"
        ...
        <button onClick={() => handleSave(key, tutor, block)} disabled={!canSave}
      ...
      <div className="max-w-[1600px] mx-auto p-2 md:p-6" style={{ background: '#fafafa', minHeight: '100%' }}>
        <div className="flex justify-end mb-4">
          <div className="flex items-center gap-1">
            <button onClick={() => { const d = new Date(todayIso + 'T00:00:00'); d.setDate(d.getDate() - 1); onDateChange(d); }}
              className="w-7 h-7 flex items-center justify-center rounded-lg transition-all hover:bg-[#f3f4f6]"
              style={{ border: '1px solid #e5e7eb', color: '#1f2937', fontSize: 18, lineHeight: 1 }}>{'<'}</button>
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl border bg-white" style={{ borderColor: '#e5e7eb' }}>
              ...
              <input type="date" value={todayIso} onChange={handleDateChange} className="text-xs font-bold outline-none bg-transparent" style={{ color: '#374151', cursor: 'pointer' }} />
            </div>
            <button onClick={() => { const d = new Date(todayIso + 'T00:00:00'); d.setDate(d.getDate() + 1); onDateChange(d); }}
              className="w-7 h-7 flex items-center justify-center rounded-lg transition-all hover:bg-[#f3f4f6]"
              style={{ border: '1px solid #e5e7eb', color: '#1f2937', fontSize: 18, lineHeight: 1 }}>{'>'}</button>
      ...
      <div style={{ maxWidth: 1600, width: '100%', margin: '0 auto', padding: '12px 24px 24px', display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
        ...
        <div className="hidden md:flex items-center gap-3 mb-4 shrink-0">
          <div className="flex items-center gap-4">
            <div>
              ...
              <p className="text-xs font-semibold" style={{ color: '#9ca3af' }}>{selectedDate.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p>
            </div>
            <div className="flex items-center gap-1">
              <button onClick={() => { const d = new Date(todayIso + 'T00:00:00'); d.setDate(d.getDate() - 1); onDateChange(d); setForms({}); }}
                className="w-7 h-7 flex items-center justify-center rounded-lg transition-all hover:bg-[#f3f3f6]"
                style={{ border: '1px solid #e5e7eb', color: '#1f2937' }}>‹</button>
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl border bg-white" style={{ borderColor: '#e5e7eb' }}>
                <CalendarIcon size={14} style={{ color: '#9ca3af' }} />
                <input type="date" value={todayIso} onChange={handleDateChange} className="text-xs font-bold outline-none bg-transparent" style={{ color: '#374151', cursor: 'pointer' }} />
              </div>
        ...
        <div className="flex md:hidden items-center justify-between mb-3 shrink-0">
          <div>
            ...
            <p className="text-[10px] font-semibold" style={{ color: '#9ca3af' }}>{selectedDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
          </div>
        ...
        {todayTutors.length === 0 ? (
          ...
          <div style={{ display: 'flex', gap: 16, flex: 1, minHeight: 0 }}>
                                    ...
                                    {hasStudents && !isOnTimeOff && session!.students.map((student: any) => (
                                      <div key={student.rowId || student.id}
                                        ...
                                        <div className="flex justify-between items-start mb-1">
                                          ...
                                          <div className="flex items-center gap-1">
                                            ...
                                            <button onClick={async e => {
                                              ...
                                              style={student.status === 'present' ? { background: '#059669', border: '1.5px solid #059669' } : { background: 'white', border: '1.5px solid #d1d5db' }}>
                                        ...
                                        <div className="flex items-center gap-1.5 mt-0.5">
                                          ...
                                          {student.seriesId && <span className="text-[8px] font-black px-1 py-0.5 rounded" style={{ background: '#ede9fe', color: '#7c3aed' }}>↺ REC</span>}
                                        </div>
                            ...
                            return (
                              <div key={block.id} className="flex-shrink-0 w-40 p-1.5"
                                ...
                                <div className="space-y-1" style={{ minHeight: 64 }}>
                                    ...
                                    <>
                                      {session!.students.map((student: any) => (
                                        <div key={student.rowId || student.id}
                                          ...
                                          style={student.status === 'no-show' ? { background: 'transparent', border: '1.5px solid #e5e7eb', opacity: 0.4 } : student.status === 'present' ? { background: '#edfaf3', border: '1.5px solid #6ee7b7' } : { background: palette.bg, border: `1.5px solid ${palette.border}` }}>
                                          <button onClick={async e => {
                                              e.stopPropagation();
                                              const next = student.status === 'present' ? 'scheduled' : 'present';
                                              await updateAttendance({ sessionId: session.id, studentId: student.id, status: next });
                                      ...
                                      {!isFull && (
                                        <button onClick={() => openForm(tutor, block.time)}
                                          className="w-full py-1 rounded-lg text-[7px] font-bold uppercase"
                                          style={{ background: 'transparent', border: '1.5px dashed #d1d5db', color: '#9ca3af' }}>
                                          + ADD
                                        </button>
                                      )}
                                  ...
                                  {isAvail && (
                                    <div onClick={() => openForm(tutor, block.time)}
                                      className="w-full h-full rounded-lg flex flex-col items-center justify-center gap-1 cursor-pointer active:scale-95"
                                  ...
                                  {isOutside && (
                                    <div className="w-full rounded-lg flex flex-col items-center justify-center gap-1"
                                      ...
                                      {isOnTimeOff ? <span className="text-[8px] font-black uppercase tracking-widest" style={{ color: '#dc2626' }}>OFF</span> : <span className="text-[8px] font-semibold uppercase tracking-wider" style={{ color: '#d1d5db' }}>—</span>}
                                    </div>
                                  )}
                                </div>
                              </div>
              ...
              {(() => {
                ...
                return (
                  <div className="md:hidden fixed inset-0 z-50 flex flex-col justify-end"
                    ...
                    <div className="rounded-t-2xl overflow-hidden" style={{ background: 'white', boxShadow: '0 -4px 24px rgba(0,0,0,0.15)' }}>
                      ...
                      <div className="flex items-center justify-between px-4 pb-3 pt-1">
                        <div>
                          ...
                          <p className="text-[11px] font-semibold mt-0.5" style={{ color: '#6b7280' }}>{tutor.name} · {block.label} ({block.display})</p>
                        </div>
                        <button onClick={() => closeForm(openKey)} className="w-8 h-8 flex items-center justify-center rounded-full" style={{ background: '#f3f4f6', color: '#6b7280' }}>
                          <X size={14} />
                        </button>
                      </div>
                      <div className="px-4 pb-6 space-y-3" data-inline-form>
                        <div className="relative" data-inline-form>
                          <input autoFocus type="text" placeholder="Student name…"
                            value={form.student ? form.student.name : form.query}
                            onChange={e => { patchForm(openKey, { query: e.target.value, student: null }); setOpenDropdown(openKey); }}
                          ...
                          {form.student && (
                            <button onMouseDown={e => { e.preventDefault(); patchForm(openKey, { student: null, query: '' }); }}
                              className="absolute right-3 top-1/2 -translate-y-1/2" style={{ color: '#9ca3af' }}>
                              <X size={14} />
                            </button>
                          )}
                          {openDropdown === openKey && hints.length > 0 && !form.student && (
                            <div data-inline-form className="absolute left-0 right-0 rounded-xl overflow-hidden"
                              style={{ bottom: 'calc(100% + 4px)', background: 'white', border: '1px solid #e5e7eb', boxShadow: '0 -4px 16px rgba(0,0,0,0.1)' }}>
                              {hints.map(s => (
                                <button key={s.id} className="w-full text-left px-4 py-3 text-sm font-semibold"
                                  style={{ color: '#111827', borderBottom: '1px solid #f3f4f6' }}
                                  onMouseDown={e => {
                                    e.preventDefault();
                                    const allTopics = [...MATH_TOPICS, ...ENG_TOPICS];
                        ...
                        <select value={form.topic} onChange={e => patchForm(openKey, { topic: e.target.value })}
                          className="w-full text-sm font-semibold rounded-xl px-4 py-3 outline-none"
                          style={{ background: '#f3f4f6', border: '1.5px solid #e5e7eb', color: '#374151' }}>
                          {topics.map(t => <option key={t} value={t}>{t}</option>)}
                          <option value="Other">Other</option>
                        </select>
                        {/* Recurring toggle mobile */}
                        <div className="flex items-center gap-2">
                          <button onClick={() => patchForm(openKey, { recurring: !form.recurring })}
                          ...
                          {form.recurring && (
                            <select value={form.recurringWeeks} onChange={e => patchForm(openKey, { recurringWeeks: Number(e.target.value) })}
                        ...
                        {form.error && <p className="text-xs font-semibold" style={{ color: '#dc2626' }}>{form.error}</p>}
                        <button onClick={() => handleSave(openKey, tutor, block)} disabled={!canSave}
                          ...
                          style={{ background: canSave ? '#6366f1' : '#e5e7eb', color: canSave ? 'white' : '#9ca3af' }}>
                          {form.saving ? <><Loader2 size={14} className="animate-spin" /> Saving…</> : form.recurring ? `Book ${form.recurringWeeks} Weeks` : 'Book Student'}
                        </button>
                      </div>
                    </div>
                  </div>
            ...
            <SidePanel
              todayIso={todayIso} sessions={sessions} tutors={tutors} daySessions={daySessions}
              dayLabel={dayLabel} pendingStudents={pendingStudents}
              setSelectedSessionWithNotes={setSelectedSessionWithNotes} refetch={refetch}
            />
          </div>
        )}
      </div>
